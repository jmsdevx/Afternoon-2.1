...
class Substitute extends Component {
    ...
    displayPlayer(){
        let showPlayers = this.props.myPlayers.map((e,i)=>{
            return <h4 key={i}>{e}
              <button id="start" onClick={()=>this.props.toCourt(i)}>Start</button> 
              <button id="sit" onClick={()=>this.props.toBench(i)}>Sit</button> 
              <button id="cut" onClick={()=>this.props.cut(i)}>Cut</button></h4>
        })
        return showPlayers
    }
    ...
    displayCourt(){
        let courtPlayers = this.props.myCourt.map((e, i)=>{
            return <h4 key={i}>{e}
            <button onClick={()=>this.props.removeCourt(i)}>Remove</button></h4>
        })
        return courtPlayers
    }
    ...
    displayBench(){
        let benchPlayers = this.props.myBench.map((e, i)=>{
            return <h4 key={i}>{e}
            <button onClick={()=>this.props.removeBench(i)}>Remove</button></h4>
        })
        return benchPlayers
    }
